...
class ToolsConsole:
    ...
    def usage():
        print((
                  '* Nguyễn An Sử Dụng %d Cách\n'
                  'Lưu ý: Nếu danh sách Proxy trống, cuộc tấn công sẽ chạy mà không cần proxy\n'
                  '      Nếu tệp Proxy không tồn tại, tập lệnh sẽ tải xuống các proxy và kiểm tra chúng.\n'
                  '      Loại Proxy 0 = Tất Cả Trong config.json\n'
                  '      SocksTypes:\n'
                  '         - 6 = RANDOM\n'
                  '         - 5 = SOCKS5\n'
                  '         - 4 = SOCKS4\n'
                  ...
                  '\n'
                  'Example:\n'
                  '   L7: python3 start.py <phương thức> <url> <kiểu vớ> <luồng> <danh sách proxy> <rpc> <thờilượng> <gỡ lỗi-tùy chọn>\n'
                  '   L4: python3 start.py <phương thức> <ip:port> <luồng> <thời lượng>\n'
                  '   L4 Proxy: python3 start.py <phương thức> <ip:port> <luồng> <thời lượng> <loại vớ> <proxydanh sách\n'
                  '   L4 Amplification: python3 %s <Phương Thức> <ip:port> <Luồng> <Thời Lượng> <Tệp Phản Xạ (Chỉ Sử Dụng Với'
                  ' Amplification)>\n') %
...
# Hàm kiểm tra người dùng được ủy quyền
...
def lag(update, context):
    ...
    try:
        ...
        if method not in Methods.ALL_METHODS:
            update.message.reply_text(f"Phương thức không hợp lệ. Các phương thức hỗ trợ: {', '.join(Methods.ALL_METHODS)}")
            return
        ...
        update.message.reply_text(f"Đã bắt đầu tấn công {target} với phương thức {method}, {threads} luồng, trong {duration} giây.")
...
def stop_attack(update, context):
    ...
    if attack_event.is_set():
        attack_event.clear()
        update.message.reply_text("Đã dừng tấn công.")
    else:
        update.message.reply_text("Không có tấn công nào đang chạy.")

# Hàm giả lập để bắt đầu tấn công (thay thế bằng logic thực tế)
def start_attack(method, target, threads, duration, event):
    logger.info(f"Bắt đầu tấn công: Method={method}, Target={target}, Threads={threads}, Duration={duration}")
    start_time = time.time()
    while time.time() - start_time < duration and event.is_set():
        logger.info(f"Đang tấn công {target} bằng phương thức {method}...")
        time.sleep(1)  
    logger.info("Tấn công kết thúc.")
...
# Chạy bot Telegram trong một luồng riêng
...
# Biến toàn cục lưu trạng thái tấn công
attack_status = {
    "is_running": False,
    "start_time": None,
    "duration": None,
...
def status(update, context):
    ...
    if attack_status["is_running"]:
        elapsed_time = time.time() - attack_status["start_time"]
        remaining_time = attack_status["duration"] - elapsed_time
        update.message.reply_text(
            f"Cuộc tấn công đang chạy:
"
            f"Phương thức: {attack_status['method']}
"
            f"Mục tiêu: {attack_status['target']}
"
            f"Thời gian còn lại: {remaining_time:.2f} giây"
        )
    else:
        update.message.reply_text("Không có cuộc tấn công nào đang chạy.")

# Hàm gửi thông báo
def send_notification(context, chat_id, message):
    context.bot.send_message(chat_id=chat_id, text=message)

# Cập nhật hàm /lag để gửi thông báo
def lag(update, context):
    user_id = update.message.from_user.id
    if not is_authorized(user_id):
        update.message.reply_text("Bạn không có quyền sử dụng lệnh này.")
    ...
    try:
        ...
        threading.Thread(target=start_attack, args=(method, target, threads, duration, attack_event)).start()
        send_notification(context, user_id, f"Cuộc tấn công đã bắt đầu: {attack_status['method']} -> {attack_status['target']}")
...
# Cập nhật hàm /stop để gửi thông báo
def stop_attack(update, context):
    user_id = update.message.from_user.id
    if not is_authorized(user_id):
        update.message.reply_text("Bạn không có quyền sử dụng lệnh này.")
    ...
    if attack_status["is_running"]:
        ...
        send_notification(context, user_id, "Cuộc tấn công đã dừng.")
...
def help_command(update, context):
    help_text = (
        "Các lệnh có sẵn:
"
        "/lag <phương thức> <mục tiêu> <luồng> <thời lượng> - Bắt đầu tấn công.
"
        "/stop - Dừng tấn công hiện tại.
"
...
# =========================
# Quản lý người dùng ủy quyền
# =========================

import os

# File lưu danh sách người dùng ủy quyền
AUTHORIZED_USERS_FILE = "authorized_users.txt"

# Hàm đọc danh sách người dùng từ file
def load_authorized_users():
    if not os.path.exists(AUTHORIZED_USERS_FILE):
        return [123456789]  # ID người dùng mặc định
    with open(AUTHORIZED_USERS_FILE, "r") as f:
        return [int(line.strip()) for line in f.readlines()]

# Hàm ghi danh sách người dùng vào file
...
# Khởi tạo danh sách người dùng ủy quyền
AUTHORIZED_USERS = load_authorized_users()

# Hàm xử lý lệnh /add_user
def add_user(update, context):
    ...
    if not context.args:
        update.message.reply_text("Sử dụng: /add_user <user_id>")
        return

    new_user_id = int(context.args[0])

    if new_user_id in AUTHORIZED_USERS:
        update.message.reply_text(f"Người dùng {new_user_id} đã có quyền.")
        return

    AUTHORIZED_USERS.append(new_user_id)
    save_authorized_users()  # Lưu danh sách vào file
    update.message.reply_text(f"Đã thêm người dùng {new_user_id} vào danh sách ủy quyền.")

# Hàm xử lý lệnh /list_users
def list_users(update, context):
    ...
    if not AUTHORIZED_USERS:
        update.message.reply_text("Danh sách người dùng ủy quyền trống.")
        return

    users_list = "
".join(str(user_id) for user_id in AUTHORIZED_USERS)
    update.message.reply_text(f"Danh sách người dùng ủy quyền:
{users_list}")

# Hàm xử lý lệnh /remove_user
def remove_user(update, context):
    ...
    if not context.args:
        update.message.reply_text("Sử dụng: /remove_user <user_id>")
        return

    remove_user_id = int(context.args[0])

    if remove_user_id not in AUTHORIZED_USERS:
        update.message.reply_text(f"Người dùng {remove_user_id} không có trong danh sách ủy quyền.")
        return

    AUTHORIZED_USERS.remove(remove_user_id)
    save_authorized_users()  # Lưu danh sách vào file
    update.message.reply_text(f"Đã xóa người dùng {remove_user_id} khỏi danh sách ủy quyền.")
